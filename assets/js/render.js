...
async function renderHomeStartup() {
    ...
    if (about) {
        ...
        if (about.hero) {
            ...
            heroDescs.forEach(el => {
                if (about.hero.description && about.hero.description.trim()) {
                    el.textContent = about.hero.description;
                }
                // If description is missing/empty, leave the hardcoded HTML text in place
            });
    ...
    if (services && services.featuredServices) {
        ...
        if (featuredContainer) {
            featuredContainer.innerHTML = services.featuredServices.map((service, index) => `
                <div class="col-lg-3 col-md-6 col-12 mb-50px">
                    <div class="accordion-item ${index === 0 ? 'active' : ''}" style="height: 100%; border: none; border-top: 1px solid rgba(255, 255, 255, 0.1); background: transparent; padding-top: 30px; display: flex; flex-direction: column;">
                        <div class="bg-img mb-25px" style="height: 180px; border-radius: 6px; width: 100%; background-image: url('${service.bgImage}'); background-size: cover; background-position: center;"></div>
                        <div class="accordion-header" id="heading${service.id}">
                            <div class="accordion-button" type="button"
                                ...
                                aria-expanded="${index === 0}" aria-controls="collapse${service.id}" style="background: transparent; color: #fff; border: 0; box-shadow: none; padding: 0;">
                                <div class="w-100">
                                    <div class="d-flex justify-content-between align-items-center mb-10px">
                                        <h6 style="color: #FF6A3A; font-weight: 600;">${service.num}</h6>
                                        <div class="icon-arrow">
                                            <svg width="18" height="18" viewBox="0 0 18 18"
                                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M13.922 4.5V11.8125C13.922 11.9244 13.8776 12.0317 13.7985 12.1108C13.7193 12.1899 13.612 12.2344 13.5002 12.2344C13.3883 12.2344 13.281 12.1899 13.2018 12.1108C13.1227 12.0317 13.0783 11.9244 13.0783 11.8125V5.51953L4.79547 13.7953C4.71715 13.8736 4.61092 13.9176 4.50015 13.9176C4.38939 13.9176 4.28316 13.8736 4.20484 13.7953C4.12652 13.717 4.08252 13.6108 4.08252 13.5C4.08252 13.3892 4.12652 13.283 4.20484 13.2047L12.4806 4.92188H6.18765C6.07577 4.92188 5.96846 4.87743 5.88934 4.79831C5.81023 4.71919 5.76578 4.61189 5.76578 4.5C5.76578 4.38811 5.81023 4.28081 5.88934 4.20169C5.96846 4.12257 6.07577 4.07812H13.5002C13.612 4.07813 13.7193 4.12257 13.7985 4.20169C13.8776 4.28081 13.922 4.38811 13.922 4.5Z"
                                                    fill="currentColor"></path>
                                            </svg>
                                        </div>
                                    </div>
                                    <h3 class="text-uppercase font-weight-700 fs-20" style="font-size: 20px; font-family: 'Unbounded', sans-serif;">${service.title}</h3>
                                </div>
                            </div>
                        </div>
                        <div id="collapse${service.id}" class="accordion-collapse collapse ${index === 0 ? 'show' : ''}"
                            aria-labelledby="heading${service.id}" data-bs-parent="#accordionExample" style="flex: 1;">
                            <div class="accordion-body pt-20px" style="padding: 0;">
                                <div class="tags text-uppercase fs-12 mb-15px">
                                    ${service.tags.map(tag => `<a href="#0" style="display: inline-block; margin-right: 10px; margin-bottom: 5px;">${tag}</a>`).join('')}
                                </div>
                                <p class="opacity-7" style="font-size: 14px; line-height: 1.6; font-family: 'Poppins', sans-serif; color: #a8a8a8;">${service.description}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    if (team) {
        ...
        if (galleryContainer) {
            let html = team.slice(0, 4).map((member, i) => `
